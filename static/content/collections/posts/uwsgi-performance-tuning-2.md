...
为了印证这个猜测，我尝试把进程数上限调到 20，发现 TPS 上升到 350，而在进程数改到 10 之后 TPS 上升到 370-390。说明**在资源有限的情况下，进程开太多确实会造成进程之间 CPU 竞争过于严重的问题**。根据这个实验结果，个人猜测 worker 数量设置到 CPU 核心数的 1 倍到 2 倍是比较合理的（没有印证，如果读者对此印证过请告诉我）。
